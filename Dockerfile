...
RUN gcloud auth activate-service-account --key-file=/usr/gcloud-service-key.json &&\
	gsutil -m cp -r /usr/src/bundle_client gs://tg-static-bucket/static/${COMMIT_REF} &&\
	gsutil -m mv -r gs://tg-static-bucket/static/public gs://tg-static-bucket/static/pub-before-${COMMIT_REF} &&\
	gsutil -m mv -r gs://tg-static-bucket/static/${COMMIT_REF} gs://tg-static-bucket/static/public &&\
